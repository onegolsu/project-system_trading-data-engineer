...
class KoreaInvestment:
    ...
    def create_order_resv(
        self, side: str, symbol: str, price: int, quantity: int, order_type: str, resv_date: datetime.date
    ) -> dict:
        """
        국내주식주문/주식주문 예약(현금)
        ...
        unpr = "0" if order_type == "01" else str(price)
        sll_buy = "01" if side == "sell" else "02"
        resv_date_str = resv_date.strftime("%Y%m%d")
    ...
    def fetch_open_order_resv(self, side="sell"):
        ...
        params = {
            "RSVN_ORD_ORD_DT": datetime.date.today().strftime("%Y%m%d"),
            "RSVN_ORD_END_DT": (datetime.date.today() + datetime.timedelta(days=30)).strftime("%Y%m%d"),
            "RSVN_ORD_SEQ": "0",
            "PDNO": "",
