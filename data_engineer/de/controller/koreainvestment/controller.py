...
from .broker import KoreaInvestment
...
class ORDER_CONTROLLER:
    ...
    class ORDER_PREPROCESSOR:
        ...
        def get_resv_order_df(self, position_df):
            resv_order_df = position_df.copy()
            resv_order_df["ResvUpperPrice"] = resv_order_df["BuyingPrice"].apply(
                lambda x: self._call_price_maker(x * 1.06)
            )
            resv_order_df["ResvLowerPrice"] = resv_order_df["BuyingPrice"].apply(
                lambda x: self._call_price_maker(x * 0.98)
            )
            return resv_order_df
